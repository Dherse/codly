...
#for (numbers, outside, smart) in ((true, false, true), (true, true, true), (false, false, true), (true, false, false)) {
  context {
    let origin = here()
    for enabled in (false, true) {
      ...
      codly.new(raw(source, lang: "js", block: true),
        indent-guides: enabled, rainbow: true, smart-indent: smart,
        number-enabled: numbers, header: [head], footer: [foot], radius: 4pt,
    ...
    context {
      ...
      for row in query(selector(<guide-row>).after(origin).before(here())) {
        runs.at(if row.value.first() { 1 } else { 0 }).push((row.value.last(), row.location().position()))
      }
...
#context {
  let original = raw.line(1, 1, "    wrapped source source source source source source", text("    wrapped source source source source source source"))
  let split = raw.line(1, 1, original.text, text(fill: red, "  ") + strong("  ") + text("wrapped source source source source source source"))
  assert.eq(measure(codly.codly-line(original), width: 90pt), measure(codly.codly-line(split), width: 90pt))
}

// Native labels, source text, and highlight character coordinates survive.
@guide-code:12 @guide-highlight
#figure(caption: [Guides])[
  #codly.new(raw("root\n    call();", lang: "js", block: true), block-label: <guide-code>, offset: 10,
...
#context {
  ...
  codly.new(raw("  " + "long_argument " * 120, block: true),
    number-enabled: false, indent-guides: (width: 2, x-offset: 0.5em))
  context {
